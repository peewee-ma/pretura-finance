...
def cmd_import_csv(args):
    ...
    if "date" in raw.columns and "time" in raw.columns:
        ...
        dt_col = next((c for c in raw.columns if "time" in c or "date" in c), raw.columns[0])
        dt = pd.to_datetime(raw[dt_col])
    dt = dt - pd.Timedelta(hours=args.tz_shift)          # Serverzeit -> Berlin
